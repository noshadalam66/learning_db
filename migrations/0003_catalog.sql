...
-- Courses.
--
-- instructor_id holds an identity.users.id but carries NO foreign key. MySQL
...
CREATE TABLE catalog.courses (
  ...
  -- identity.users.id - deliberately not a foreign key (see above).
  ...
  -- Denormalised; refreshed by catalog.refresh_course_rollup().
  duration_minutes  INT NOT NULL DEFAULT 0,
  lesson_count      INT NOT NULL DEFAULT 0,
  -- Denormalised; refreshed by catalog.refresh_course_rating().
...
CREATE TABLE catalog.course_tags (
  ...
  CONSTRAINT fk_course_tags_course FOREIGN KEY (course_id)
    REFERENCES catalog.courses (id) ON DELETE CASCADE,
  CONSTRAINT fk_course_tags_tag FOREIGN KEY (tag_id)
    REFERENCES catalog.tags (id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE catalog.modules (
  ...
  -- transient duplicate the way it can in PostgreSQL. catalog.reorder_lessons()
...
CREATE TABLE catalog.lessons (
  ...
  CONSTRAINT fk_lessons_module FOREIGN KEY (module_id)
    REFERENCES catalog.modules (id) ON DELETE CASCADE,
...
CREATE TRIGGER catalog.trg_categories_no_self_parent_ins
  BEFORE INSERT ON catalog.categories FOR EACH ROW
BEGIN
...
CREATE TRIGGER catalog.trg_categories_no_self_parent_upd
  BEFORE UPDATE ON catalog.categories FOR EACH ROW
BEGIN
  IF NEW.parent_id IS NOT NULL AND NEW.parent_id = NEW.id THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'a category cannot be its own parent';
  END IF;
...
CREATE TRIGGER catalog.trg_courses_touch
  BEFORE UPDATE ON catalog.courses FOR EACH ROW
BEGIN SET NEW.updated_at = UTC_TIMESTAMP(3); END$$

CREATE TRIGGER catalog.trg_modules_touch
  BEFORE UPDATE ON catalog.modules FOR EACH ROW
BEGIN SET NEW.updated_at = UTC_TIMESTAMP(3); END$$

CREATE TRIGGER catalog.trg_reviews_touch
  BEFORE UPDATE ON catalog.course_reviews FOR EACH ROW
BEGIN SET NEW.updated_at = UTC_TIMESTAMP(3); END$$
...
CREATE TRIGGER catalog.trg_lessons_touch_ins
  BEFORE INSERT ON catalog.lessons FOR EACH ROW
BEGIN
...
CREATE TRIGGER catalog.trg_lessons_touch_upd
  BEFORE UPDATE ON catalog.lessons FOR EACH ROW
BEGIN
  DECLARE module_course CHAR(36) CHARACTER SET ascii;
