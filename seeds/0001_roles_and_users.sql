...
SELECT u.id, r.id
  ...
  JOIN identity_users u ON u.email = v.email
  JOIN identity_roles r ON r.name = v.role_name
ON DUPLICATE KEY UPDATE granted_at = identity_user_roles.granted_at;

-- ---------------------------------------------------------------------------
-- Categories and tags
-- ---------------------------------------------------------------------------
INSERT INTO catalog_categories (id, slug, name, description, `position`) VALUES
